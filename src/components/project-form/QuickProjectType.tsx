...
const QuickProjectType = ({ onSuggestionsApplied, contextData }: QuickProjectTypeProps) => {
  ...
  const handleGetSuggestions = async () => {
    ...
    try {
      ...
      // Create a description based on selections
      const description = `${selectedStyle} ${selectedType} project`;
      const response = await descriptionAnalysisService.analyzeDescription(description);
      
      if (response && response.success) {
        setSuggestions(response);
        console.log('✅ Quick type suggestions received:', response);
      } else {
        setError("Failed to get suggestions. Please try again.");
        console.error('❌ Quick type suggestions error:', response);
      }
    } catch (error) {
      console.error('❌ Error getting quick type suggestions:', error);
  ...
  const handleApplySuggestions = () => {
    if (suggestions) {
      ...
      console.log('🎯 Applied quick suggestions to form');
    }
  };
              ...
              {suggestions.suggested_materials && suggestions.suggested_materials.length > 0 && (
                <div>
                  ...
                  <p className="text-muted-foreground">
                    {suggestions.suggested_materials.slice(0, 3).map(m => m.name).join(", ")}
                  </p>
                </div>
              )}
